...
//        item.setLocationId(itemDto.getLocationId());
    ...
    @Override
    public Page<Item> findOffers(Person person, int pageNo, int pageSize, String sortBy, boolean started, boolean notStarted, boolean virtualPayment, boolean physicalPayment, String searchKey, boolean excludeFinalized) {
        List<Item> itemList = new ArrayList<>();
        try {
            for(AuctionOffer auctionOffer : person.getAuctionOffers()) {
                Item item = auctionOffer.getItem();
